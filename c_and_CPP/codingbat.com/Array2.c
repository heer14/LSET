...
void countEvens(){
    int testCases[8][5] = {{2, 1, 2, 3, 4},{2,2,0,-1,-1},{1,3,5,-1,-1},{-1,-1,-1,-1,-1},{11,9,0,1,-1},{2,11,9,0,-1},{2,-1,-1,-1,-1},{2,5,12,-1,-1}};
    for(int i=0;i<8;i++){
        int count =0;
        printf("[");
        for(int j=0;j<5 ;j++)
        {
            if (testCases[i][j]!= -1) printf("%d , ", testCases[i][j]);
            if(testCases[i][j] % 2 ==0) count++;
        }
        printf("]-> %d\n",count);
        }
}
